...
int main(int argc, char* argv[]) {
    ...
    // Promotion test: wait for IO Pattern cold eviction to cull MEMORY replicas,
    // then re-read seeded keys to trigger promotion-on-hit.
    if (FLAGS_promotion_test_wait_sec > 0 && !seed_outcome.keys.empty()) {
        ...
        const auto wait_end = Clock::now() +
            std::chrono::seconds(FLAGS_promotion_test_wait_sec);
        while (Clock::now() < wait_end) {
            ...
            for (size_t i = 0; i < seed_outcome.keys.size(); ++i) {
                ...
                const bool is_hot =
                    offload_all ? false : seed_outcome.hot[i];
                snapshot.keys.push_back(KeyMetrics{
                    .object = {.tenant_id = tenant, .key = seed_outcome.keys[i]},
            ...
            snapshot.storage.push_back(StorageMetric{
                .source_id = FLAGS_node_id, .observed_at_ns = now_ns,
                .tier = CacheTier::kL1Host, .memory_used_ratio = 0.5F});
                ...
                if (setup_ret != 0) {
                    ...
                    while (Clock::now() < poll_end) {
                        ...
                        for (const auto& key : seed_outcome.keys) {
                            const auto descs = wait_client->get_replica_desc(key);
                        ...
                        const auto elapsed_s = std::chrono::duration_cast<
                            std::chrono::seconds>(Clock::now() - poll_start)
                                                   .count();
                        LOG(INFO) << "[PREFETCH-TEST] local_disk-ready keys="
                    ...
                    if (ready < seed_outcome.keys.size()) {
                        LOG(WARNING) << "[PREFETCH-TEST] LOCAL_DISK readiness "
                                        "TIMEOUT: ready="
                                     << ready << "/" << seed_outcome.keys.size()
            ...
            for (uint64_t rep = 0; rep < FLAGS_prefetch_repeat_reports; ++rep) {
                ...
                for (size_t i = 0; i < prefetch_keys.size(); ++i) {
                    snapshot.keys.push_back(KeyMetrics{
                        ...
                        .prefix_depth = 0,
                        .prefix_fanout = 32U,
                        .match_length = static_cast<uint32_t>(
                            FLAGS_prefetch_match_length),
                        .continuous_prefix_length = static_cast<uint32_t>(
                            FLAGS_prefetch_match_length),
                        .recompute_cost = 0.0F,
                        .replica_tiers = tiers,
                        .request_priority = 1,
        ...
        if (buffer) {
            ...
            if (ret == 0) {
                ...
                for (const auto& key : seed_outcome.keys) {
                    for (int a = 0; a < 5; ++a) {
                        int64_t got = client->get_into(key, buffer, FLAGS_value_size);
                        if (got >= 0) ++promo_hits; else ++promo_misses;
                    }
                }
    ...
    if (FLAGS_post_promo_evict_wait_sec > 0 && !seed_outcome.keys.empty()) {
        ...
        const auto recycle_end = Clock::now() +
    ...
    if (FLAGS_prefetch_fake_local_report && !real_seed_mode) {
        ...
        for (uint64_t rep = 0; rep < FLAGS_prefetch_repeat_reports; ++rep) {
            ...
            snapshot.storage.push_back(StorageMetric{
                .source_id = FLAGS_node_id, .observed_at_ns = now_ns,
                .tier = CacheTier::kL1Host, .memory_used_ratio = 0.1F});
            send_snapshot(snapshot);
